...
#' @param level.cnfd Numeric (float) corresponding to confidence level \strong{default}: \code{level.cnfd = 0.95}
#' @param lw.tail Logical, indicating the way how to calculate \code{qt} quantile function for the \eqn{t}-distribuion,
#'  it is inherited from \code{\link[stats:TDist]{stats::qt}}, if \code{TRUE} (\strong{default}), probabilities are \eqn{P[X\leq x]},
#'  otherwise, \eqn{P[X>x]}
#' @param separate Logical, whether to separate the mean value and the uncertainty, if \code{TRUE}, the result is
