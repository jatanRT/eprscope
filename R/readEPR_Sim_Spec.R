...
#'   )
#' #
#' ## load simulated EPR spectrum from `.csv` file
#' ## generated by the `plot_eval_ExpSim_app()` function
#' test.sim.df <-
#'   readEPR_Sim_Spec(
#'     "testicek_table.csv",
#'     col.names.sim = c(
...
#'   )
#'  #
#'  ## such `.csv` can be also used to load
#'  ## simulated EPR spectrum/spectra in order
#'  ## to get quantitative information
#'  ## by the `quantify_EPR_Sim_series()` function
#' }
#'
#'
#' @export
...
origin.sim <- tolower(origin.sim)
#
if (origin.sim == "easyspin") {
  spectrum.data <- fread(
    file = path_to_ASC,
    sep = "auto",
