...
#'   and it is ONLY ACTIVATED IN CASE KINETIC MODEL FITTING PROCEDURE (see also \code{\link{eval_kinR_EPR_modelFit}}).
...
#'   following table =>
#'   \tabular{ccl}{
#'   \strong{`model.react`} \tab | \tab \strong{Essetial `kin.params` components} \cr
#'   -------------------- \tab | \tab ------------------- \cr
...
#'   using the experimental data as well as time corresponding to time series experiment. The essential two columns
#'   are described by character strings like bellow (see the last two arguments). The \code{data.expr} should be used
#'   in case if experimental time ... tbc
