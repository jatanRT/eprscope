...
#'   If \code{element.react = FALSE} (the model reaction is not considered as the elemental one) therefore,
...
#' @param element.react Logical, description...TBC...
...
#'   as well as on the \code{element.react}. If \code{element.react = FALSE} additional parameterized partial
...
eval_kinR_ODE_model <- function(model.react = "(n=1)R --> [k1] B", ## e.g. n = 1,2
                                ...
                                element.react = TRUE, ## can be `FALSE` or `TRUE`
                                kin.params = c(
                                  k1 = 0.001,
                                  qvar0R = 0.02
