...
#'   the radical(s) (labeled as "R"). Profiles are provided by the numeric solution of the rate equations
#'   by the \strong{O}rdinary \strong{D}ifferential \strong{E}quations (ODE).
#'
#'
#' @details
...
#'   e.g. from rev.
#'   comproportionation (\eqn{\text{A}^{++} + \text{A}^0 \xrightleftharpoons ~ 2\text{R}^{.+}},
...
#'   Defined/Allowed values are integers e.g. 1,2,3...etc.
...
#'   must be incl. in \code{kin.params} (see also \code{kin.params} description).
...
#'   is \strong{FALSE} and it is ONLY ACTIVATED (\code{model.expr.diff = TRUE}) IN THE CASE WHEN KINETIC MODEL
...
#'   The initial values are denoted as \code{qvar0X} (e.g. qvar0R for radical or qvar0A for reactant `A`).
#'   The components of \code{kin.params} named numeric vector depend on \code{model.react}
#'   as well as on the \code{elementary.react}. If \code{elementary.react = FALSE} additional parameters like partial
#'   reaction orders (\code{alpha} and/or \code{beta} and/or \code{gamma}) must be defined within \code{kin.params},
#'   like summarized in the following table =>
#'   \tabular{ll}{
#'   \strong{`model.react` string} \tab \strong{Essential `kin.params` components} \cr
...
eval_kinR_ODE_model <- function(model.react = "(n=1)R --> [k1] B", ## e.g. n = 1,2
  ...
  stoichiom_coeff <- function(expression, coeff = "n") {
    if (coeff == "n") {
      nml.string <- stringr::str_extract(expression, pattern = "\\(n=[[:digit:]]\\)")
    }
    if (coeff == "m") {
      nml.string <- stringr::str_extract(expression, pattern = "\\(m=[[:digit:]]\\)")
    }
    if (coeff == "l"){
      nml.string <- stringr::str_extract(expression, pattern = "\\(l=[[:digit:]]\\)")
    }
    model.react.nml <- stringr::str_extract(nml.string, pattern = "[[:digit:]]")
    model.react.nml <- as.numeric(model.react.nml)
    #
    return(model.react.nml)
    #
  }
  #
