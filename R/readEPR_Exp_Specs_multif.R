...
#' @param col.names Character string vector, corresponding to column/variable names \strong{for individual file}
#'   (see also \code{\link{readEPR_Exp_Specs}}). A safe rule of thumb is to use column names including the physical
#'   quantity notation with its units, \code{Quantity_Unit} like \code{"B_G"}, \code{"RF_MHz"}, \code{"Bsim_mT"} (e.g. pointing
...
#' @param qValues Numeric vector of Q-values (sensitivity factors to normalize EPR intensities) either automatically loaded
