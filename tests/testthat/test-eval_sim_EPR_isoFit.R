...
test_that("The parameters determined from the expr. EPR spectrum
  ...
  expect_lt(aminoxyl.rad.data.simFit.a$min.rss[[1]], 1.3e-7) # original intensity in the range of 1e-5, 2e-4
  ## Linewidths => expr. + sim. fit. comparison (difference lower than 0.03)  =>
  expect_lte(abs(weight.DeltaB.sim.fit - mean.DeltaB.expr), 0.025) # in mT
