...
test_that("The parameters determined from the expr. EPR spectrum
    ...
    aminoxyl.rad.data.simFit.a <-
      eval_sim_EPR_isoFit(data.spectr.expr = aminoxyl.rad.data.a,
                          nu.GHz = mw.freq.GHz,
                          B.unit = "mT",
                          lineG.content = 0.9,
    ...
    #
    ## mean value of the Gaussian and Lorentzian Linewidths
    mean.DeltaB.sim.fit <-
      round(
        mean(
      c(aminoxyl.rad.data.simFit.a$best.fit.params[[1]][2],
      aminoxyl.rad.data.simFit.a$best.fit.params[[1]][3])
      ),
      digits = 2
      )
    #
    ...
    ## Linewidths => expr. + sim. fit. comparison (difference lower than ...)  =>
    expect_lt((mean.DeltaB.sim.fit-mean.DeltaB.expr),0.04) # in mT
