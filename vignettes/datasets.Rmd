...
DT::datatable(isotopes_ds,
  ...
  caption = "Dataset with the following variables/columns: Proton number, isotope, stability (either stable, 
  STB or radio-active, RA), isotope name, nuclear spin quantum number, nuclear g-value, natural abundance 
  in %, nuclear quadrupolar moment in Barns and the ENDOR/Larmor frequency in MHz at 0.35 T. The negative 
  sign of Larmor frequency values points to clockwise precession direction according to convention."
)
...
DT::datatable(solvents_ds,
  ...
  solubility in g/(100 g of water), relative electric permittivity, flash point in °C and dynamic 
  viscosity in cp."
)
...
These are the [ASCII text](https://www.computerhope.com/jargon/a/ascii.htm) data, coming from the EPR spectrometer and correspond either to EPR spectrum table[^1] or the accompanying text files including the (instrumental) parameters of the recorded spectrum (with `.par` or `.DSC` / `.dsc` extensions). Additionally, the `{eprscope}` package contains `.mat` (*Matlab*) files from *EasySpin* simulations as well as outputs from DFT quantum chemical computations or related structural data with `.inp.log.zip` and `.sdf` extensions. All files used in this package are summarized within the following table:

[^1]: Usually the following variables/columns are included in the ASCII tables: "**index**", "**Field [G]**"/"**X [G]**" and "**Intensity []**"/"**Intensity**"
...
+-------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------------+
| `Aminoxyl_radical_a.DSC`      | ASCII **text file incl. parameters** to record the EPR spectrum of an aminoxyl/nitroxyl radical derivative corresponding to the above-listed `Aminoxyl_radical_a.txt`                                                                                       | -   used for testing of `eval_sim_EPR_iso()` and `eval_sim_EPR_isoFit()`                                          |
+-------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------------+
...
+-------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------------+
| `TMPD.sdf`                    | **Structured data file** (based MOL-file format) containing structure of the N,N,N',N'-Tetramethyl-p-phenylenediamine (TMPD) radical cation. Structure (optimized geometry) was calculated by the B3LYP/6-31+G(d,p) DFT level.                              | -   used as an example to demonstrate the `draw_molecule_by_rcdk()` functionality                                 |
+-------------------------------+-------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------+-------------------------------------------------------------------------------------------------------------------+
