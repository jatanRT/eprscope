...
The `dplyr::select()` function can be also applied to drop columns as well as to select columns, resulting in data frame, only by a specific conditions or with specific names (see the related [documentation](https://dplyr.tidyverse.org/reference/select.html)).
...
triarylam.decay.df <- readEPR_Exp_Specs_kin(
...
#
# preview:
triarylam.decay.df$df

```

Such "complex" loading and reading is required because the kinetic data is relatively large and therefore in a compressed form stored within the package. Otherwise, all one have to do is just to define the file path for both parameter as well as ASCII table files (see also the documentation of the `readEPR_Exp_Specs_kin()`). Because the `triarylam.decay.df` is list (see also below), it consists of two compotents: the data frame `df` and the vector of corrected `time` at which the middle of the CW EPR spectrum appears.
