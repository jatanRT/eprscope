...
One can preview the structure of data frame by the `str()` function from the `{utils}` 📦:
...
We can immediately recognize that all three columns are **num**eric and the dimension (which can be also figured out by the `dim()` function) reads 2401 (rows) $\small \times$ 3 (variables/columns). The dollar sign `$` in front of each column vector represent an operator to select the entire data frame column (variable), namely:
...
Selection/Subset by single square brackets results ...
