...
# where `file` is the argument of `load_data_example` function
...
one can try to fit the *Area (Double Integral) vs time* relation, by the above-mentioned kinetic model, to estimate the kinetic rate constant $\small k_1$ [^4]:
...
As shown above (see Section \@ref(data-frames)), to select/view a specific data frame column one can use two types of operators: `$` (dollar sign) and `[]` or `[[]]` (square brackets). The same applies for lists as it was depicted in several code snippets/code chunks above, using the `$` operator. For example to ...TBC... TODO !!
