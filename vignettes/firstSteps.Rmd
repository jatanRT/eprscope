...
[^4]: Because the double integral is directly proportional to the concentration of radicals $\mathrm{R}$ for a quick preview (or comparison with other kinetic data) we can use such estimation based on integrals/areas. Consequently, the unit of the rate constant (corresponding to radical recombination) will be expressed in: $[k_1] = (\mathrm{p.d.u.})^{-1}\, \mathrm{s}^{-1}$.
