...
1.  The native pipe operator is automatically loaded, starting the ![](RcoreLogo.png){width="16" height="12"} console/Rstudio IDE and does not require any additional 📦, whereas the `%>%` is included in the `{magrittr}` (or `{tidyverse}` bundle) and therefore they must be loaded before its use.

...

... settings for the pipe operator within the Rstudio `Tools` ➝ `Global Options...` ➝ `Code` (left-hand panel) and finally activate or deactivate the checkbox "**Use native pipe operator, \|\> (requires R 4.1+)**" like depicted in \@ref(fig:rstudio-pipe-setup).
