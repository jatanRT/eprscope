...
To avoid such a behavior one has to define a `list` as shown below. The length of a vector can be figured out by the `length()` function, counting the number of vector components:
...
One can preview the **str**ucture of a data frame by the `str()` function from the `{utils}` 📦:
...
3.  Pipe operators differ in placeholders (like `.` or `_` ) call, where one doesn't have to repeat the argument of a function (see also below ):
...
Last example of data handling (see below), using the `{dplyr}` and the pipe operator, will show the processing and analysis of EPR time series (kinetics). In the first step, the "kinetic" data must be loaded:
