...
Packages are fundamental units of shareable code. They may bundle functions, sample code, datasets, documentation and tests [@GfG2023; @wickham2023]. By default, during the ![](RcoreLogo.png){width="16" height="12"} installation, only a couple of packages are set up (e.g. `{base}` , `{datasets}` , `{graphics}` , `{grdevices}` , `{stats}` , `{utils}` ). These are also immediately available, once you start the ![](RcoreLogo.png){width="16" height="12"} console or *Rstudio* IDE (Integrated Development Invironment). Additional packages must by installed and loaded explicitly in order to be utilized. To load such packages, the function `library()` has to be executed. It actually refers to the (default) place (folder) where the 📦 is stored on your computer, such as loading the collection of essential data science packages ([{tidyverse}](https://www.tidyverse.org/), already mentioned above):
...
In order to select/convert a data frame row into vector, one has to, first of all, `unlist()` the row (because it possesses the form of data frame) and finally remove the names (if the named vector is not desired):
...
A data frame can be also created, by definition (see above), using the equally long vectors (refer to the vector definitions in Section \@ref(vectors-mats) and at beginning of the vignette), like:
...
## Basic Statistical Measures of Data Frame Columns

The basic statistical measures of a data frame (and its columns) are available by the `summary()` command/function:
...
Additionally, for each column one can also obtain (by using a loop `sapply()` ) the [sample variance](https://www.r-tutor.com/elementary-statistics/numerical-measures/variance) and the [standard deviation](https://www.r-tutor.com/elementary-statistics/numerical-measures/standard-deviation):
...
```{r rstudio-pipe-setup,echo = FALSE,out.width = "91%",fig.cap = "Pipe operator setup with the Rstudio Options."}
...
Last example of data handling (see below), using the `{dplyr}` and the pipe operator, will show the processing and analysis of EPR time series (kinetics). In the first step, the "kinetic" must be loaded from the `.zip` file :
...
#
# ascii table data in the form of `.zip`:
...
triarylam.decay.integs.01 <- 
  triarylam.decay.dat$df |>
    ...
    # summarize the sigmoid integral max. value
...
[^4]: Because the double integral is directly proportional to the concentration of radicals $\ce{R}$ for a quick preview (or comparison with other kinetic data) we can use such estimation based on integrals/areas. Consequently, the unit of the rate constant (corresponding to radical recombination) will be expressed in: $[k_1] = (\mathrm{p.d.u.})^{-1}\, \mathrm{s}^{-1}$.
...
Obviously, according to statistical measures of non-linear regression/fitting parameters, the second-order kinetic model is suitable for description of triarylamine radical cation decay. This is also supported by the analysis of residuals/errors of the model/fit:
...
However, it is evident that the variance (and the standard deviation) of residuals is slightly higher along the fitted/predicted values in comparison to residual/error values:
...
To **select the** `N.converg` **component by the** `$` **operator, we do not have to remember its name**. In most of the IDEs, by writing the commands/scripts, there is an automatic code completion. So, once you start to put `$` operator just behind the list variable, all list elements appear in a popup window in order to select the desired one[^5].

[^5]: The same actually applies also for data frames and columns.
...
In terms of previous definitions and list selections, creation of list is quite straightforward. As an example we may define system of nuclei, interacting with unpaired electron to simulate the $\small \ce{TMPD^{.+}}$ EPR spectrum, having the structure already depicted in `vignette("functionality")` :
