...
In order to avoid such behavior one has to define a `list` as shown below. The length of the a vector can be figured out by the `length()` function, counting the number of vector components:

```{r vector-length}

# dermine vector length (`comb.norm`):
length(comb.norm)
...
#
# remove the second and fifth  element:
comb.norm[-c(2,5)]
#
...
The basic arithmetic operations on vectors are also performed elementwise:
...
# through all elements (i) of `B.central.vec`:
vec.g.iso.a <- sapply(
...
# preview (comparison):
vec.g.iso.a
identical(vec.g.iso.a,vec.g.iso.b) # Are vectors identical ?
...
# define vector of hypothetical g-values:
...
# select element in 2nd row
# and 3rd column within `mat.01`:
mat.01[2,3]
#
...
identical(
  diag(nrow = 3,ncol = 3),
  round(A_1 %*% A,digits = 8) # rounding on 8 decim. places
) # Are matrices identical ?

```

The inverse matrix calculations requires following condition:
...
Data frame is one of most important structure/object in ![](RcoreLogo.png){width="16" height="12"} statistical language to store the tabular data/datasets. It is similar to matrix (because it is represented by the 2-dimensional scheme), but unlike the matrices it can take different data types in lists (vide infra) of vectors with the equal lengths, corresponding **to columns/variables**. The second dimension is defined by the **rows/observations**, consisting of vector elements (with equal indices).
