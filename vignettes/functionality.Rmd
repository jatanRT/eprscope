...
# Parameter codes (WinEPR acq. spectrometer software)
# can be also obtained by `readEPR_param_slct()`
# function with the codes described in the corresponding
# documentation.
#
# Following parameters are required (`req.params`) =>
# conversion time (RCT) in milliseconds, receiver gain (RRG) unitless,
# sweep with (HSW) in Gauss, number of points (RES) and number
# of scans/accumulations (JSD).
#
# creating a vector of codes
params.code <- c("RCT", "RRG", "HSW", "RES", "JSD")
#
# iterating over its components by the `sapply()` function
req.params <-
  sapply(
    params.code,
    function(c) {
...
adv.norm.constant <- quantify_EPR_Norm_const(
  conv.time.ms = req.params[1],
  Nscans = req.params[5],
  Npoints = req.params[4],
  rg = req.params[2],
  rg.unit = "Unitless",
  Bsw = req.params[3]
)
...
In order to explore the EPR spectrum in details (to read values, zoom or save the spectrum), function `plot_EPR_Specs2D_interact()` is applied (consult the corresponding function example). Additionally, it can be also used for the series of overlaid EPR spectra similarly like `plot_EPR_Specs()`. Additional interactive 3D-plotting of EPR spectra, especially those recorded as time series, like during the kinetic or variable temperature experiments are provided by the `plot_EPR_Specs3D_interact()` function.
...
2.  Linewidth, for derivative line form expressed by $\small \Delta B_{\mathrm{pp}}$, that is the difference between $\small B_{\mathrm{max}}$ and $\small B_{\mathrm{min}}$ corresponding to position of maximal and minimal intensity, respectively. For the integrated spectral form the linewidth is expressed by $\small FWHM$ ("full width at half-maximum" or "full width at half-height").
...
\end{equation}
```
where the $h$ and $\mu_{\mathrm{B}}$ are Plank's and Bohr magneton constants, respectively. Therefore, it is determined only by the microwave frequency ($\nu$) and the magnetic flux density ($\small B_0$) of the spectrum mid-point where $\small \mathrm{d}I_{\mathrm{EPR}}/\mathrm{d}B = 0$ (in the case of CW EPR spectra, see the function `eval_gFactor_Spec()`).
...
sim.tmpd.iso <-
  eval_sim_EPR_iso(
    ...
    # natural abundance of the nuclei:
    natur.abund = TRUE,
...
present_EPR_Sim_Spec(
  ...
  # `plot_theme_NoY_ticks` theme from `{eprscope}` + `{ggplot2}` function
  plot_theme_NoY_ticks(legend.text = element_text(size = 13))

```

Although the simulated $\ce{TMPD^{.+}}$ EPR spectrum is quite close to the experimental one, there are still some apparent differences. Therefore, to gather a better simulated spectrum a fitting/optimization function `eval_sim_EPR_isoFit()` may be required.
...
The relative error of the cumulative trapezoidal integration is minimal, usually falling into the range of $\small \langle 1-5\rangle\,\%$ or even lower depending on the spectrum resolution [@LibreMath2023; @epperson2013intro]. If the initial EPR spectrum displays low signal-to-noise ratio, the integral required for the quantitative analysis looses its sigmoid-shape (example of such shape see below). Therefore, in such case, the best option is to simulate the EPR spectrum and to evaluate the integral from that spectrum (see e.g. `quantify_EPR_Sim_series()` function). The upside of this method consists in graphical representation of the corresponding integrals, i.e. the integration process may be visually controlled by the same manner like within the EPR spectrometer acquisitions/processing software (see also `plot_EPR_Specs_integ()` function below). The integration of the EPR spectral data is essentially provided by the `eval_integ_EPR_Spec()` and its application to the experimental $\small \ce{TMPD^{.+}}$ spectrum is documented by the following scripts ➨
