...
1.  `.asc`, `.txt`, `.csv` , `.DTA` and `.spc` to read spectra from the EPR instrument/spectrometer, where the latter two correspond to binary file formats (the remaining three are ASCII text-based formats), which also require presence of the following files in order to load those spectra
...
3.  `.mat` coming from *Maltab* to gather all the parameters and variables related to simulated EPR spectrum returned from the [*EasySpin*](https://easyspin.org/) *toolbox*
