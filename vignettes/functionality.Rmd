...
```{r spectral-data-norm0102-visual}
...
## to save figure `ggsave()` function from `{ggplot2}` is applied
# ggsave("path_to_image_file",
...
In order to explore the EPR spectrum in details, function `plot_EPR_Specs2D_interact()` is applied. Additionally, it can be also used for series of EPR spectra similarly like `plot_EPR_Specs()`. The last spectrum with advanced normalization ➨
