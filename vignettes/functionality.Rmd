...
readEPR_params_tabs(path_to_dsc_par = tmpd.params.file,
                    ...
                    interact = "params")
#
# or by function `datatable` from the `DT` package
...
In order to explore the EPR spectrum in details (to read values, zoom or save the spectrum), function `plot_EPR_Specs2D_interact()` is applied. Additionally, it can be also used for the series of EPR spectra similarly like `plot_EPR_Specs()`. The last spectrum with the advanced normalization can be plotted like ➨
...
tmpd.interact.epr.spec <- plot_EPR_Specs2D_interact(data.spectra = tmpd.data.norm.02)
#
# preview
tmpd.interact.epr.spec

```

Additional interactive 3D-plotting of EPR spectra, especially those recorded as time series, like during the kinetic or variable temperature experiments is described in other vignettes and provided by the `plot_EPR_Specs3D_interact()` function.
...
# with their corresponding intensities automaticaly
