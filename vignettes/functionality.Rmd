...
tmpd.plot.norm.02 <- 
  ...
  ggplot2::labs(title = "Advanced Normalization",y = NULL)
#
...
\end{equation}
```
where the $h$ and $\mu_{\mathrm{B}}$ are Plank and Bohr magneton constants, respectively. Therefore, it is determined only by microwave frequency ($\nu$) and the magnetic flux density ($B_0$) of the spectrum mid-point (see function `eval_gFactor_Spec()`).
...
The `TRUE` value represents a perfect agreement between the theoretically (`2.003168`) and experimentally (`2.00304`) determined $g_{\mathrm{iso}}$-values and thus confirmed the position of the $\ce{TMPD^{.+}}$ EPR spectrum. Its difference from the free electron $g\approx 2.00232$ is a consequence of a [spin-orbit coupling](https://chem.libretexts.org/Bookshelves/Physical_and_Theoretical_Chemistry_Textbook_Maps/Electron_Paramagnetic_Resonance_(Jenschke)/03%3A_Electron_Zeeman_Interaction/3.01%3A_Physical_origin_of_the_(g)_shift) of the unpaired electron within the described radical cation structure.
...
# selected B regions as a list of three elements
B.regions.mT <- list(
  c(349.677, 349.977),
  c(349.177, 349.492),
  c(350.822, 351.072)
...
# through each element of `B.regions.mT`
