...
The *Wuster's* reagent got his name according to *C. Wuster* who discovered the oxidation products of *p*-phenylenediamine derivatives [@GramppGpda2005]. It is a suitable compound to demonstrate the feasibility of Electron paramagnetic resonance (EPR) combined with the *in situ* electrochemistry/voltammetry because of the one-electron oxidation [@PetrAse1996]:
...
# The intensity is normalized by Q value (sensitivity factor)
# and the number of scans (`Nscans`). From the previous parameter/info 
# reading we know: Q = 3500, Nscans = 20.
...
Visualization and/or conversion of data frames (tables) into several formats like `.html` , `.pdf` , `.docx` or `.pptx`. can be provided by the mostly used ["table" packages](https://rfortherestofus.com/2019/11/how-to-make-beautiful-tables-in-r/). Among them, libraries like [{DT}](https://rstudio.github.io/DT/), [{flextable}](https://davidgohel.github.io/flextable/index.html), [{gt}](https://gt.rstudio.com/) and [{kableExtra}](https://haozhu233.github.io/kableExtra/) (see Section \@ref(hf-structure) below) are often used for publication ready table visualization. Example of the last data frame previewed by `{flextable}` is shown below.

```{r spectral-data-read-norm-flextable}

# visualization of the first 10 rows by `{flextable}` in `booktabs` (LaTeX-like) theme
# (`theme_booktabs()`) and pipe oerator "%>%" which is an expression 
# of a sequence of operations transforming an object
  flextable::flextable(head(tmpd.data.norm.02, n = 10)) %>%
    flextable::theme_booktabs()

```

In order to process data frames in other software tools different from `r icons::bioicons("R")` they may be exported into universal `.csv` table format like by `base::write.csv()` function ➨
...
If one wants to save the previous Figure \@ref(fig:spectral-data-norm0102-visual) the function `ggplot2::ggsave()` can be used ➨
...
In order to explore the EPR spectrum in details (to read values, zoom or save the spectrum), function `plot_EPR_Specs2D_interact()` is applied (consult the corresponding function example). Additionally, it can be also used for the series of overayed EPR spectra similarly like `plot_EPR_Specs()`. Additional interactive 3D-plotting of EPR spectra, especially those recorded as time series, like during the kinetic or variable temperature experiments is described in other vignette and provided by the `plot_EPR_Specs3D_interact()` function.

```{r interact-spectral-data-visual}

# inteactive plot preview based on `{plotly}`
...
4.  Single or double integral depending on the original spectral line form ➨ either integrated or derivative, respectively. This is directly proportional to number of paramagnetic species/radicals.
...
# the g-value is obtained from the interactive plot above
tmpd.g.iso.spec <- 
...
# of its principal components (g1,g2,g3) summarized in output files
...
# => list of three elements/regions
B.regions.mT <- list(
  c(349.677, 349.977),
  c(349.177, 349.492),
  c(350.822, 351.072)
...
#
# all Delta Bpp as vector created by the iteration through 
# all elements of `B.regions.mT` (by the `sapply()` function)
...
## Hyperfine (HF) Structure and Simulation {#hf-structure}

The **HF** structure represents one of the most important characteristic because it reflects the distribution of unpaired electron within the radical/paramagnetic molecule. At least some of the hyperfine splitting constants ($a$, HFSCs, distance between the main lines within the pattern) may be estimated from the interactive EPR spectrum above. For the subsequent simulation of the EPR spectrum these are simply converted by `convert_a_mT_2A()` into hyperfine coupling constants in $\mathrm{MHz}$ ($A$, HFCCs) by the following relation (Equation \@ref(eq:hfatoA)) ➨

```{=tex}
\begin{equation}
 A = \frac{a\,g\,\mu_{\mathrm{B}}}{h}
 (\#eq:hfatoA)
 \end{equation}
```
...
#
# converssion into A (in MHz) incl. g.iso value 
...
According to literature [@GramppGpda2005] and the $\ce{TMPD^{.+}}$ structure depicted above the interacting nuclei may be divided into three groups of equivalent nuclei: $2\,\times\,\ce{^{14}N}$, $4\,\times\,\ce{^1H}$ (aromatic protons) and $12\,\times\,\ce{^1H}$ (methyl protons). One can consider the DFT calculations (already mentioned above) to obtain all $A$ (HFCCs). The optimized geometry/structure of $\ce{TMPD^{.+}}$ is depicted in Figure \@ref(fig:dft-tmpd-structure).

```{r dft-tmpd-structure,echo=FALSE,fig.cap="DFT optimized structure of TMPD radical cation with atom numbering inherited from Gaussian 16 output file."}

knitr::include_graphics("imgs/TMPD_EPR_DFT_radCat_ab.png",dpi = 200)

```

The function `rearrange_aAiso_QCHorgau()` extracts the $A$ values from the Gaussian or [ORCA](https://www.orcasoftware.de/tutorials_orca/index.html) output files and calculates the mean values corresponding to selected groups of equivalent nuclei which are defined by the `nuclei.list.slct` argument. Therefore, according to numbering shown in Figure \@ref(fig:dft-tmpd-structure) ➨

```{r As-DFT-from-gaussian}

# The same built-in file (`TMPDAradCatEPRa.inp.log.zip`)
# as for extraction of g-value is used (see above).
symmetry.As.df <-
...
# (see also https://haozhu233.github.io/kableExtra/awesome_table_in_html.html)
...
From the previous analysis it turns out that the highest HF-coupling/splitting constants are related to the interaction of unpaired electron with the nitrogen and methyl-proton nuclei. Therefore the highest spin density is located on the periphery of the $\ce{TMPD^{.+}}$ molecule. This may be also supported by the simulation (theoretical calculation) of the EPR spectrum (which is calculated by the `eval_sim_EPR_iso()` function) by taking into account the following parameters, accordingly ➨
...
-   content of the Gaussian lineshape in the overall linear combination of both Gaussian and Lorenztian theoretical spectral forms (so called pseudo-Voigtian lineshape) ➨ `lineG.content`
...
where after the arrow arguments of the `eval_sim_EPR_iso()` function are shown. By considering all the above-listed parameters and the estimated linewidth `tmpd.delta.Bpp.mean` ($0.5\,\mathrm{G}$) as well as the hyperfine coupling constants, either directly estimated from the spectrum (`A.guess.MHz`) or from the quantum chemical calculations (`symmetry.As.df`) and/or taken from the literature [@GramppGpda2005], the EPR spectrum of $\ce{TMPD^{.+}}$ can be simulated as follows ➨

```{r tmpd-radcat-simulation}

# isotropic simulation of the TMPD radical cation where the instrumental
# parameters are taken from the corresponding `.par` file:
# `tmpd.params.file`,
sim.tmpd.iso <- 
  eval_sim_EPR_iso(g.iso = tmpd.g.iso.spec,
                   instrum.params = NULL,
                   path_to_dsc_par = tmpd.params.file,
                   origin = "winepr",
                   B.unit = "mT",
                   nuclear.system = list(list("14N",2,18.6), # A(2 x 14N) = 18.6 MHz
                                         list("1H",4,5.5), # A(4 x 1H) = 5.5 MHz
                                         list("1H",12,19.6)), # A(12 x 1H) = 19.6 MHz
                   natur.abund = TRUE, # consider natural abundance of the nuclei above
                   lineGL.DeltaB = list(0.05,0.05), # Gaussian (G) & Lorentzian (L) linewidth (mT)
                   lineG.content = 0.5) # pseudo-Voigt line shape with 0.5 G + 0.5 L
#
# The output is either interactive spectrum plot or list of plot
# with the simulated spectrum data frame.
#
...
Afterwards, the simulated EPR spectrum can be presented/compared with the experimental one by the `presentEPR_Sim_Spec()` function, which is essentially based on `{ggplot2}` package and therefore can be combined with the [additional {ggplot2} functions](https://ggplot2.tidyverse.org/reference/index.html) as shown below ➨

```{r tmpd-radcat-sim-expr-compar, fig.cap="Comparison of simulated and experiental EPR spectrum of $\\ce{TMPD^{.+}}$."}

# The intesinty of the simulated spectrum
# is automatically scalled onto the experimental one.
present_EPR_Sim_Spec(data.spectr.expr = tmpd.data.norm.02,
                     data.spectr.sim = sim.tmpd.iso$df,
                     B.unit = "mT") +
  plot_theme_NoY_ticks() + # `plot_theme_NoY_ticks` theme from `{eprscope}` pckg.
  scale_x_continuous(sec.axis = dup_axis(name = "",labels = NULL)) +
  theme(legend.text = element_text(size = 13)) # `{ggplot2}` functions

```

Although the simulated $\ce{TMPD^{.+}}$ EPR spectrum is quite close to the experimental one, there are still some apparent differences. Therefore, in order to better fit the simulated spectrum onto the experimental one a fitting/optimization function `eval_sim_EPR_isoFit()` may be used. More information can be found in `vignette("EPR_Simulations")`.

## Integrating the EPR Spectrum

Integration process in the analysis of EPR spectra is undoubtedly important operation because it mirrors the amount of paramagnetic centers/radicals within the sample. However, it is heavily dependent on the initial form of the EPR spectrum, i.e. whether it is present either in derivative (usually in CW EPR) or already integrated (usually in 1D pulsed EPR) form/shape. Therefore, the spectrum has to be integrated either twice or only once, respectively. The core-function used for the integration is based on the cumulative trapezoidal approximation available from the [{pracma}](https://github.com/cran/pracma) 📦 `pracma::cumtrapz()` (see also [@Rpracma2023; @LibreMath2023; @Math24net2023; @epperson2013intro]).

The disadvantage of the cumulative trapezoidal method lies in poorly resolved EPR spectra leading to a higher integration errors. However, if the spectral resolution (number of points) is relatively high (e.g. $\geq 1024\,\mathrm{points/sweep}$) the relative error is minimal usually falling into the range of $\langle 1-5\rangle\,\%$ or even lower [@LibreMath2023; @epperson2013intro]. Additionaly, if the initial EPR spectrum displays low signal-to-noise ratio, the integral required for the quantitative analysis looses its sigmoid-shape (see below). Therefore, in such case, the best option is to simulate the EPR spectrum and to evaluate the integral from that spectrum (see e.g. `quantify_EPR_Sim_series()` function). Thus, the big advantage of this method consists in graphical representation of the corresponding integrals, i.e. the integration process may be visually controlled by the same manner like within the EPR spectrometer acquisitions/processing software (see also `plot_EPR_Specs_integ()` function below). The integration of the EPR spectral data is essentially provided by the `eval_integ_EPR_Spec()` and its application to the experimental $\ce{TMPD^{.+}}$ spectrum is documented by the following scripts ➨

```{r tmpd-basic-epr-integ}

# Basic integration without basiline correction
# during the first integration.
tmpd.data.norm.02.integ <- 
  eval_integ_EPR_Spec(data.spectr = tmpd.data.norm.02,
                      B = "B_mT",
                      B.unit = "mT",
                      sigmoid.integ = TRUE)
#
# The integration returns the original data frame
# with the additional integrals `single_Integ` + `sigmoid_Integ` 
# preview
head(tmpd.data.norm.02.integ)

```

Both integrals, within the entire $B$-range, can be visualized by `plot_EPR_Specs_integ()` function from `Visualization and Graphics` family:

```{r tmpd-basic-epr-integ-visual,fig.cap = "Single (`single_Integ`) and double (`sigmoid_Integ`) integrated EPR spectrum of $\\ce{TMPD^{.+}}$."}
...
plot_EPR_Specs_integ(tmpd.data.norm.02.integ,
...
Obviously, the baseline of the single and double integrated $\ce{TMPD^{.+}}$ EPR spectrum is somewhat distorted and must be corrected. To fix the baseline distortion the `eval_integ_EPR_Spec()` function contains corresponding arguments which control the baseline approximation, under the complex peak, by the polynomial fit and provide its subtraction from the single integrated spectrum to evaluate the double/sigmoid integral more accurately ➨

```{r tmpd-correct-epr-integ, fig.cap = "Corrected single and double integrated EPR spectrum of $\\ce{TMPD^{.+}}$ (see also Figure \\@ref(fig:tmpd-basic-epr-integ-visual))."}
...
tmpd.data.norm.02.integCorr <- 
  eval_integ_EPR_Spec(data.spectr = tmpd.data.norm.02,
                      ...
                      B.unit = "mT",
                      correct.integ = TRUE,
                      BpeaKlim = c(344.5,355.5), # peak region to correct the baseline
                      poly.degree = 3, # polynomial degree to fit baseline under the peak region
                      sigmoid.integ = TRUE)
#
# Integrals preview
# data frame column of corrected integral
# is denoted as `single_Integ_correct`
plot_EPR_Specs_integ(tmpd.data.norm.02.integCorr,
                     slct.integs = c("single_Integ_correct",
                                     "sigmoid_Integ"),
                     B = "B_mT",
                     B.unit = "mT",
                     separate.integs = TRUE,
...
Maximal value of the double/sigmoid integral `r round(max(tmpd.data.norm.02.integCorr$sigmoid_Integ),digits = 4)` can be used for the estimation of $\ce{TMPD^{.+}}$ amount within the sample.
