...
Such processing (with the elapsed time of $\approx 220\,\mathrm{s}$ on 10-years old 4-core-CPU workstation) will not only return the above-defined list (`tmpd.epr.iso.fit` , see also documentation of the `eval_sim_EPR_isoFit_space()`), however also an animation, showing the progress of simulated EPR spectrum fit at each `Evaluation` :
...
tmpd.epr.iso.fit$optim.space.plot

```

```{r tmpd-isoFit-space-params-plot,echo=FALSE,fig.align='center',fig.cap="Progress of the EPR simulation parameter optimization for the $\\small \\ce{TMPD^{.+}}$ experimental spectrum, corresponding to above-depicted animation."}
